...
public class InvokeFormat : InvokeDotnet
{
    ...
    [Parameter(ValueFromPipeline = true, ValueFromPipelineByPropertyName = true, Mandatory = false, HelpMessage = "A space separated list of diagnostic ids to ignore when fixing code style or 3rd party issues.", ParameterSetName = "Format")]
    public string[] ExcludeDiagnostics { get; set; }

    [Parameter(ValueFromPipeline = true, ValueFromPipelineByPropertyName = true, Mandatory = false, HelpMessage = " Verify no formatting changes would be performed. Terminates with a non-zero exit code if any files would have been formatted.", ParameterSetName = "Format")]
    public SwitchParameter VerifyNoChanges { get; set; } = false;

    [Parameter(ValueFromPipeline = true, ValueFromPipelineByPropertyName = true, Mandatory = false, HelpMessage = "A list of relative file or folder paths to include in formatting. All files are formatted if empty.", ParameterSetName = "Format")]
    public string[] Include { get; set; } = Array.Empty<string>();

    [Parameter(ValueFromPipeline = true, ValueFromPipelineByPropertyName = true, Mandatory = false, HelpMessage = "A list of relative file or folder paths to exclude from formatting.", ParameterSetName = "Format")]
